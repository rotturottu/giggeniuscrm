...
export default function TaskBoard({ isPaidUser = false }) {
  ...
  // FIXED: Added safe check to prevent .map of undefined/non-array
  ...
  // FIXED: Added safe check for status filtering
  const tasksByStatus = Object.fromEntries(
    STATUS_COLS.map(col => [
      col.key, 
      boardTasks.filter(t => (t.status || 'todo') === col.key)
    ])
  );
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap gap-3 items-center">
        ...
        <Button onClick={() => { setEditingTask(null); setShowTaskForm(true); }} className="bg-gradient-to-r from-blue-600 to-purple-600">
          <Plus className="w-4 h-4 mr-2" /> Add Task
        </Button>
      </div>

      {view === 'list' && <TaskListView tasks={filteredTasks} onEdit={openEdit} isPaidUser={isPaidUser} />}
      {view === 'calendar' && <TaskCalendarView tasks={filteredTasks} onEdit={openEdit} />}
      {view === 'board' && (
              ...
              <Card className="bg-gray-50 shadow-none border-none">
                <CardHeader className="p-3">
                  <CardTitle className="text-sm flex items-center justify-between">
                    <span className="font-bold text-gray-500 uppercase text-[10px] tracking-widest">{col.label}</span>
                    <Badge variant="secondary" className="bg-white">{tasksByStatus[col.key]?.length || 0}</Badge>
                  </CardTitle>
                </CardHeader>
              </Card>
      ...
      <TaskForm open={showTaskForm} onClose={() => { setShowTaskForm(false); setEditingTask(null); }} task={editingTask} isPaidUser={isPaidUser} />
    </div>
  );
}