...
export default function Conversations() {
  ...
  const { data: drafts = [] } = useQuery({
    ...
    queryFn: async () => {
      const res = await base44.entities.Conversation.filter({ status: 'draft', sender_email: myEmail }, '-last_message_at');
      return Array.isArray(res) ? res : [];
    },
    enabled: !!myEmail
  });
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: (newConv) => {
      ...
      // CRITICAL: Clear view then switch to the new one to ensure no old messages leak
  ...
  const handleAction = (status = 'active') => {
    if (!composeData.to || !composeData.message) return toast.error("Please fill in all fields");
    if (!myEmail) return toast.error("Session error. Please re-login.");
  ...
  const filteredConversations = conversations.filter(conv => {
    ...
    return (conv?.contact_name || '').toLowerCase().includes(search) || (conv?.contact_email || '').toLowerCase().includes(search);
  });
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex gap-2">
                  ...
                  drafts.map(d => (
                    <div key={d.id} className="flex items-center hover:bg-slate-50 rounded-lg px-2 group">
                      <DropdownMenuItem className="flex-1 cursor-pointer py-3" onClick={() => {
                        setSelectedConversation(null); // Clear active window
                      ...
                      <Trash2 className="w-4 h-4 text-slate-300 hover:text-red-500 cursor-pointer" onClick={(e) => { e.stopPropagation(); deleteDraftMutation.mutate(d.id); }} />
                    </div>
                  ))
                }
              </DropdownMenuContent>
            ...
            <Button onClick={() => { 
              setSelectedConversation(null); // Clear view on compose
              setComposeData({ to: '', subject: '', message: '', id: null }); 
              setComposeOpen(true); 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <Card className="p-4 border-none shadow-sm bg-white">
              <div className="space-y-4">
                ...
                {isLoading ? <div className="flex justify-center py-20"><Loader2 className="animate-spin text-indigo-500" /></div> : 
                    <ConversationList conversations={filteredConversations} selectedId={selectedConversation?.id} onSelect={(c) => {
                        setSelectedConversation(null); // Force unmount old chat
                        setTimeout(() => setSelectedConversation(c), 10); // Mount new chat
                    }} />
                }
              </div>
            </Card>
          ...
          <div className="lg:col-span-8">
            {selectedConversation ? (
              <ConversationDetail 
                key={selectedConversation.id} // Forces React to destroy previous messages
                conversation={selectedConversation} 
                onDeleteSuccess={() => setSelectedConversation(null)}
              />
            ) : (
              <Card className="h-full min-h-[550px] flex items-center justify-center bg-white border-none shadow-sm text-center p-8">
                <div className="max-w-xs space-y-3">
                  ...
                  <p className="font-bold text-slate-600 text-lg">Select a contact to begin</p>
                </div>
              </Card>
            )}
          </div>
      ...
      <Dialog open={composeOpen} onOpenChange={setComposeOpen}>
        <DialogContent className="sm:max-w-[550px] p-0 overflow-hidden rounded-2xl border-none shadow-2xl">
          <DialogHeader className="bg-slate-900 text-white p-6">
            ...
            <DialogDescription className="text-slate-400">Start a private conversation.</DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter className="p-6 bg-slate-50/50 border-t border-slate-100">
            <div className="flex gap-3 w-full">
              ...
              <Button variant="outline" onClick={() => handleAction('draft')} className="h-12 border-slate-200 bg-white rounded-xl text-slate-600 px-6 font-bold">
