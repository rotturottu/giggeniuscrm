...
def init_db():
    ...
    # UPDATED Invoices Table: Added document_name, signing_date, and details columns
    ...
    tables = ['departments', 'employees', 'contacts', 'project_tasks', 'projects', 'campaigns', 'time_entries', 'deals']
    for table in tables:
        # ... (rest of your existing table logic stays the same)
...
# FIX: Added User Profile endpoint to stop 405 error
...
def handle_base44_list_create(entity_name):
    if request.method == 'OPTIONS': return jsonify({"status": "ok"}), 200
    table_map = {'Department': 'departments', 'Employee': 'employees', 'Contact': 'contacts', 'Task': 'project_tasks', 'ProjectTask': 'project_tasks', 'Invoice': 'invoices', 'Campaign': 'campaigns', 'Project': 'projects', 'TimeEntry': 'time_entries', 'Deal': 'deals'}
    ...
    if request.method == 'GET':
        ...
        # Dynamic filter for document types (Contract, Quote, etc)
    ...
    if request.method == 'POST':
        ...
        cols, placeholders = ', '.join(cleaned.keys()), ', '.join(['?'] * len(cleaned))
        c.execute(f"INSERT INTO {table_name} ({cols}) VALUES ({placeholders})", tuple(cleaned.values()))
        conn.commit()
        cleaned['id'] = c.lastrowid
        return jsonify(cleaned), 201

# ... (Keep your PUT/DELETE and Login/Register routes as they are)
...
def register():
    ...
    try:
        ...
        conn = sqlite3.connect('giggenius.db')
        conn.execute("INSERT INTO users (first_name, last_name, email, password) VALUES (?, ?, ?, ?)", (data['firstName'], data['lastName'], data['email'], hashed_pw))
        conn.commit()
